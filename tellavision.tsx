...
const obstructionsFrom = (markup) => (markup || [])
  ...
  .map((m, i) => {
    ...
    const n = typeof m.clear === "number" ? m.clear : parseFloat(m.clear);
    const clear = (isFinite(n) && n >= 0) ? n : CLEARANCE.obstruction;
    return {
      ...
      clear,
      clearRaw: m.clear,        // what the user actually typed, for the field
    };
  })
...
// A photograph of a wall is a trapezoid, not an elevation — you never shoot
// square-on. Calibrating a trapezoid with two points is right in one spot and
// progressively wrong everywhere else, which on a tool people drill from is the
// worst kind of bug: nothing looks broken.
//
// The fix is a real homography and an inverse-mapped resample. This CANNOT be a
// CSS transform: those are affine, and affine cannot express perspective.
//
// Note a homography needs the reference rectangle's PROPORTIONS, not just one
// side — four corners of an unknown rectangle are consistent with infinitely
// many shapes. Every reference offered therefore has both dimensions known.

// Gaussian elimination with partial pivoting. n is small and fixed (8), so the
// naive version is the right one.
const solveLinear = (A, b) => {
  ...
  for (let c = 0; c < n; c++) {
    ...
    if (Math.abs(M[piv][c]) < 1e-12) return null;          // singular — degenerate quad
    [M[c], M[piv]] = [M[piv], M[c]];
    for (let r = 0; r < n; r++) {
      if (r === c) continue;
      const f = M[r][c] / M[c][c];
...
// H maps src -> dst. Returned as 9 numbers, h33 fixed at 1.
...
// How far from square-on the shot is. 0 = perfectly rectangular; the ratio of
// opposite-edge lengths is what tells you the far edge is being reconstructed
// from fewer pixels than the near one.
const quadObliquity = (q) => {
  const d = (a, b) => Math.hypot(q[b][0] - q[a][0], q[b][1] - q[a][1]);
  const top = d(0, 1), bottom = d(3, 2), left = d(0, 3), right = d(1, 2);
...
// Everything the UI needs to warn about, computed before any pixels move.
const RECTIFY_OBLIQUITY_WARN = 0.35;   // far edge ~26% shorter than the near one
const RECTIFY_COVERAGE_WARN = 0.04;    // reference smaller than 4% of the frame
const rectifyChecks = (quad, natW, natH) => {
  const notes = [];
  const ob = quadObliquity(quad);
  const cov = quadArea(quad) / Math.max(1, natW * natH);
  if (ob > RECTIFY_OBLIQUITY_WARN) notes.push(`Shot from a steep angle (${Math.round(ob * 100)}% edge difference). The far side of the wall is rebuilt from fewer pixels — stand squarer if you can.`);
  if (cov < RECTIFY_COVERAGE_WARN) notes.push(`The reference fills only ${(cov * 100).toFixed(1)}% of the frame. Tap error multiplies across the wall — use the largest known rectangle in shot.`);
  return { obliquity: ob, coverage: cov, notes };
};

// Plan the output: where the whole photo lands once the quad is squared up.
// The reference is mapped to realW x realH inches at `ppi`, then the image's own
// corners are mapped through the same homography to find the bounding box, so
// the rest of the wall survives — rectifying only the quad would crop to the TV.
const planRectify = (quad, natW, natH, realW, realH, maxPx = UNDERLAY_MAX_PX) => {
  if (!(realW > 0) || !(realH > 0)) return null;
  let ppi = Math.hypot(quad[1][0] - quad[0][0], quad[1][1] - quad[0][1]) / realW;
  if (!isFinite(ppi) || ppi <= 0) return null;
  for (let pass = 0; pass < 8; pass++) {
    const dst = [[0, 0], [realW * ppi, 0], [realW * ppi, realH * ppi], [0, realH * ppi]];
    const H = solveHomography(quad, dst);
    if (!H) return null;
    const pts = [[0, 0], [natW, 0], [natW, natH], [0, natH]].map(([x, y]) => applyH(H, x, y));
    ...
    if (long <= maxPx) {
      return { H, ppi, outW, outH, minX, minY,
               inv: solveHomography(dst, quad),
               refPx: { x: -minX, y: -minY, w: realW * ppi, h: realH * ppi } };
    }
    ppi *= (maxPx / long) * 0.98;   // shrink and re-plan; the bbox is not linear in ppi
  }
  return null;
};

// Inverse-mapped resample with bilinear sampling. Every OUTPUT pixel asks where
// it came from in the source, which is what keeps the result hole-free — mapping
// source pixels forward would scatter them and leave gaps in the stretched
// region. ~3M pixels at the size cap, a few hundred ms, once per photograph.
const rectifyBitmap = (imgSrc, plan) => new Promise((resolve, reject) => {
  const img = new Image();
  ...
  img.onload = () => {
    try {
      ...
      const S0 = sctx.getImageData(0, 0, sc.width, sc.height);
      const sp = S0.data, sw = sc.width, sh = sc.height;

      const { outW, outH, minX, minY, inv } = plan;
      const dc = document.createElement("canvas");
      dc.width = outW; dc.height = outH;
      const dctx = dc.getContext("2d");
      const D = dctx.createImageData(outW, outH);
      const dp = D.data;
      ...
      for (let v = 0; v < outH; v++) {
        ...
        for (let u = 0; u < outW; u++) {
          ...
          const o = (v * outW + u) * 4;
          if (!(sx >= 0 && sy >= 0 && sx <= sw - 1 && sy <= sh - 1)) { dp[o + 3] = 0; continue; }
          const x0 = sx | 0, y0 = sy | 0;
          const x1 = x0 + 1 < sw ? x0 + 1 : x0, y1 = y0 + 1 < sh ? y0 + 1 : y0;
          const fx = sx - x0, fy = sy - y0;
          const w00 = (1 - fx) * (1 - fy), w10 = fx * (1 - fy), w01 = (1 - fx) * fy, w11 = fx * fy;
          const o00 = (y0 * sw + x0) * 4, o10 = (y0 * sw + x1) * 4, o01 = (y1 * sw + x0) * 4, o11 = (y1 * sw + x1) * 4;
          dp[o]     = sp[o00]     * w00 + sp[o10]     * w10 + sp[o01]     * w01 + sp[o11]     * w11;
          dp[o + 1] = sp[o00 + 1] * w00 + sp[o10 + 1] * w10 + sp[o01 + 1] * w01 + sp[o11 + 1] * w11;
          dp[o + 2] = sp[o00 + 2] * w00 + sp[o10 + 2] * w10 + sp[o01 + 2] * w01 + sp[o11 + 2] * w11;
          dp[o + 3] = 255;
        }
      }
      dctx.putImageData(D, 0, 0);
      // JPEG has no alpha, so the region outside the original frame would come
      // back black. Paint white under it first — it reads as paper, and the
      // trace palette already assumes a white ground.
      const flat = document.createElement("canvas");
      flat.width = outW; flat.height = outH;
      const fctx = flat.getContext("2d");
      fctx.fillStyle = "#FFFFFF"; fctx.fillRect(0, 0, outW, outH);
      fctx.drawImage(dc, 0, 0);
      resolve({ src: flat.toDataURL("image/jpeg", UNDERLAY_JPEG_Q), natW: outW, natH: outH });
...
// the palette being drawn uses for linework, so a single stroke comes out
// near-white on the blueprint and near-black on the printed sheet.
//
// Resolution never touches the stored item: `color: "auto"` is what goes into
// localStorage and into exported JSON, so a design saved on one theme opens
// correctly on another — and an old design carrying a literal "#FFFFFF" is
// still drawn in that exact white, because the user may have meant it.
const MARKUP_AUTO = "auto";
// The swatch row. MARKUP_COLORS stays the list of FIXED colours, so a saved
// mkColor from before this existed still validates against it.
const MARKUP_SWATCHES = [MARKUP_AUTO, ...MARKUP_COLORS];
// Strokes drawn before the colour picker existed carry no `color` at all.
const MARKUP_DEFAULT_COLOR = MARKUP_COLORS[0];

const resolveMarkupColor = (color, P) =>
  color === MARKUP_AUTO ? ((P && P.line) || MARKUP_DEFAULT_COLOR)
                        : (color || MARKUP_DEFAULT_COLOR);

// --- contrast ------------------------------------------------------------
// WCAG relative luminance. Used only to answer one question — will this stroke
// be visible on this ground — never to grade the UI, which is measured in a
// browser against real computed styles.
const hexToRgb = (hex) => {
...
// Deliberately NOT the WCAG 3:1 figure. A 2px saturated amber redline reads
// perfectly well on white at 1.83:1 because hue carries it, and green (1.91)
// and blue (2.75) are no different — holding markup to the text threshold
// would flag four of the six swatches and the warning would become noise
// nobody reads. 1.5 catches the one thing that actually goes wrong: a stroke
// drawn in essentially the same colour as its own paper. Red sits at 3.55 and
// near-black at 18.9, so the gap either side of the line is wide.
const MARKUP_MIN_CONTRAST = 1.5;

// Will this stroke colour still be visible on that palette's ground?
// `auto` is safe by construction — it IS the palette's linework colour.
const markupColorSafeOn = (color, P) => {
  if (color === MARKUP_AUTO) return true;
  const r = contrastRatio(resolveMarkupColor(color, P), (P && P.canvas) || "#FFFFFF");
  return r === null ? true : r >= MARKUP_MIN_CONTRAST;
};

// Strokes that would come out invisible if the drawing were rendered on `P`.
// Blanking patches are excluded — they are painted in the paper colour on
// purpose. Nothing here rewrites anything; it only reports.
const markupContrastRisk = (items, P) =>
  (items || [])
    .map((m, i) => ({ m, i }))
    .filter(({ m }) => m && m.type !== "mask" && m.type !== "obst" && (m.pts || []).length)
    .filter(({ m }) => !markupColorSafeOn(m.color || MARKUP_DEFAULT_COLOR, P))
    .map(({ m, i }) => ({
      id: m.id ?? i,
      index: i,
      type: m.type,
      color: resolveMarkupColor(m.color, P),
      label: m.type === "text" && m.text ? `TEXT "${m.text}"` : (TOOL_LABEL[m.type] || String(m.type || "").toUpperCase()),
    }));
...
const buildDXF = (S, layout) => {
  ...
  (S.markup || []).forEach((m) => {
    ...
    if (m.type === "pen") {
      ...
      // Was falling through to the line branch and exporting as a diagonal.
      rect("OBSTRUCTION", Math.min(a.x, b.x), Math.min(a.y, b.y), Math.abs(b.x - a.x), Math.abs(b.y - a.y));
      if (m.text) text("OBSTRUCTION", Math.min(a.x, b.x) + 1, Math.min(a.y, b.y) + 1, 2, String(m.text).toUpperCase());
    } else if (m.type === "text") {
...
// Markup arrives from a file, so it is untrusted: every field is checked and
// anything unrecognised is dropped rather than trusted into state. This matters
// more than it used to — an obstruction is not decoration, it moves the
// recommended mount height, so a malformed one would silently change geometry.
const MARKUP_IMPORT_TYPES = ["pen", "line", "arrow", "rect", "obst", "text", "measure", "mask"];
const cleanImportedMarkup = (arr) => {
  if (!Array.isArray(arr)) return null;
  const pt = (q) => (q && typeof q.x === "number" && isFinite(q.x) && typeof q.y === "number" && isFinite(q.y))
    ? { x: q.x, y: q.y } : null;
  return arr.map((m, i) => {
    if (!m || typeof m !== "object") return null;
    if (!MARKUP_IMPORT_TYPES.includes(m.type)) return null;
    const pts = Array.isArray(m.pts) ? m.pts.map(pt).filter(Boolean) : [];
    if (pts.length < (m.type === "text" ? 1 : 2)) return null;
    const out = {
      id: typeof m.id === "string" && m.id ? m.id : `im${i}`,
      type: m.type, pts,
      color: MARKUP_SWATCHES.includes(m.color) ? m.color : MARKUP_DEFAULT_COLOR,
      w: MARKUP_WIDTHS.includes(m.w) ? m.w : MARKUP_WIDTHS[1],
    };
    if (typeof m.text === "string" && m.text) out.text = m.text.slice(0, 120);
    if (m.type === "obst" && m.clear != null) {
      const c = typeof m.clear === "number" ? m.clear : parseFloat(m.clear);
      if (isFinite(c) && c >= 0 && c <= 120) out.clear = c;
    }
    return out;
  }).filter(Boolean);
};
...
// Preferred order for the diagnostics panel. Anything not listed still shows,
// after these — a new group can never go missing from the panel again.
const DIAG_GROUP_ORDER = ["golden", "format", "snap", "markup", "ink", "interop", "invariant"];
...
const runSelfTests = () => {
  ...
  // ---- markup ink: legibility on whatever paper it lands on ----
  {
    const near = (a, b) => Math.abs(a - b) < 0.01;
    T("ink", "contrast ratio: identical colours 1.0, black on white 21.0", (() => {
      const same = contrastRatio("#FFFFFF", "#FFFFFF");
      const max = contrastRatio("#000000", "#FFFFFF");
      return near(same, 1) && near(max, 21) && contrastRatio("#fff", "#000") === contrastRatio("#000000", "#ffffff");
    })());
    T("ink", "bad hex is reported as unknown, never as a false pass/fail",
      contrastRatio("chartreuse", "#FFFFFF") === null && relLuminance("#GGG") === null);

    // The threshold is the whole design: it has to separate "the same colour as
    // the paper" from "a saturated redline that reads fine". These are the six
    // fixed swatches against the white export sheet.
    const onPrint = MARKUP_COLORS.map(c => contrastRatio(c, PRINT_PALETTE.canvas));
    T("ink", "1.5 threshold splits the swatches where it should — white 1.00, amber 1.83, red 3.55",
      near(onPrint[4], 1) && near(onPrint[1], 1.83) && near(onPrint[0], 3.55) && near(onPrint[5], 18.88),
      onPrint.map((r, i) => `${MARKUP_COLORS[i]}=${r.toFixed(2)}`).join(" "));
    T("ink", "on the export sheet only white is flagged — the saturated colours all survive",
      MARKUP_COLORS.filter(c => !markupColorSafeOn(c, PRINT_PALETTE)).join() === "#FFFFFF");
    T("ink", "on the dark theme only near-black is flagged",
      MARKUP_COLORS.filter(c => !markupColorSafeOn(c, SCREEN_PALETTE)).join() === "#111111");

    // The point of `auto`: there is no palette on which it is the wrong choice.
    T("ink", "auto is legible on every palette, print included",
      [SCREEN_PALETTE, CYANOTYPE_PALETTE, PAPER_PALETTE, PRINT_PALETTE, tracePalette(SCREEN_PALETTE)]
        .every(P => markupColorSafeOn(MARKUP_AUTO, P)
                 && contrastRatio(resolveMarkupColor(MARKUP_AUTO, P), P.canvas) >= MARKUP_MIN_CONTRAST));
    T("ink", "auto resolves per palette — near-white on cyanotype, near-black on print",
      resolveMarkupColor(MARKUP_AUTO, CYANOTYPE_PALETTE) === "#FFFFFF"
      && resolveMarkupColor(MARKUP_AUTO, PRINT_PALETTE) === "#102A43");

    // Persistence contract. Old designs must not be reinterpreted.
    T("ink", "a stored hex is returned untouched, on every palette",
      [SCREEN_PALETTE, CYANOTYPE_PALETTE, PRINT_PALETTE].every(P => resolveMarkupColor("#FFFFFF", P) === "#FFFFFF"));
    T("ink", "a stroke saved before the colour picker still draws in the original red",
      resolveMarkupColor(undefined, PRINT_PALETTE) === MARKUP_DEFAULT_COLOR
      && resolveMarkupColor(null, CYANOTYPE_PALETTE) === MARKUP_DEFAULT_COLOR);
    T("ink", "auto survives a JSON round-trip as the sentinel, not as a colour", (() => {
      const back = JSON.parse(JSON.stringify([{ id: "a", type: "pen", color: MARKUP_AUTO, pts: [{ x: 0, y: 0 }, { x: 5, y: 5 }] }]));
      return back[0].color === MARKUP_AUTO && resolveMarkupColor(back[0].color, PRINT_PALETTE) === PRINT_PALETTE.line;
    })());

    // The export guard.
    const mk = [
      { id: "w1", type: "pen", color: "#FFFFFF", pts: [{ x: 0, y: 0 }, { x: 9, y: 9 }] },
      { id: "r1", type: "arrow", color: "#FF3B30", pts: [{ x: 1, y: 1 }, { x: 8, y: 8 }] },
      { id: "m1", type: "mask", color: "#FFFFFF", pts: [{ x: 0, y: 0 }, { x: 4, y: 4 }] },
      { id: "t1", type: "text", color: "#FFFFFF", text: "VERIFY", pts: [{ x: 2, y: 2 }] },
      { id: "a1", type: "line", color: MARKUP_AUTO, pts: [{ x: 3, y: 3 }, { x: 7, y: 7 }] },
      { id: "e1", type: "pen", color: "#FFFFFF", pts: [] },
    ];
    T("ink", "the export guard names the white strokes and nothing else", (() => {
      const risk = markupContrastRisk(mk, PRINT_PALETTE);
      return risk.length === 2 && risk[0].id === "w1" && risk[1].id === "t1" && risk[1].label === 'TEXT "VERIFY"';
    })());
    T("ink", "blanking patches are never flagged — they are painted in the paper colour on purpose",
      !markupContrastRisk(mk, PRINT_PALETTE).some(r => r.type === "mask"));
    T("ink", "a pointless empty stroke is not reported",
      !markupContrastRisk(mk, PRINT_PALETTE).some(r => r.id === "e1"));
    T("ink", "risk indices address the original array, so the recolour hits the right strokes", (() => {
      const risk = markupContrastRisk(mk, PRINT_PALETTE);
      return risk.every(r => mk[r.index].id === r.id);
    })());
    T("ink", "white markup is a risk on paper and safe on the blueprint — the ground is what decides",
      markupContrastRisk(mk, PRINT_PALETTE).length === 2
      && markupContrastRisk(mk, CYANOTYPE_PALETTE).length === 0);
    T("ink", "an all-auto drawing is clean against every palette", (() => {
      const auto = mk.filter(m => m.pts.length).map(m => ({ ...m, color: MARKUP_AUTO }));
      return [SCREEN_PALETTE, CYANOTYPE_PALETTE, PAPER_PALETTE, PRINT_PALETTE]
        .every(P => markupContrastRisk(auto, P).length === 0);
    })());
    T("ink", "the guard reports without touching the drawing", (() => {
      const before = JSON.stringify(mk);
      markupContrastRisk(mk, PRINT_PALETTE);
      markupContrastRisk(mk, SCREEN_PALETTE);
      return JSON.stringify(mk) === before;
    })());
  }
  ...
  // A theme must be colour ONLY. The stress sweep renders every configuration
  // against SCREEN_PALETTE alone, and that is only safe while a palette cannot
  // move anything — otherwise each new theme would need its own 125 configs.
  // pillStyle is the one palette key that touches drawing code (filled vs
  // outline pills), so this pins the invariant rather than assuming it.
  T("invariant", "Themes change colour only, never geometry", (() => {
    const st = {
      wallW: 120, wallH: 108, hasFireplace: true, fbOpeningW: 40, fbOpeningH: 30, fbOffsetIn: 0,
      hasMantel: true, mantelH: 54, mantelDepth: 8, brand: "Sony", selectedSize: 75,
      mountSystem: "sanus", dispUnits: "ftin", heightRef: "center", override: "",
      showVesa: true, showOutlet: true, showLowVolt: true, showBoxDims: true, showTvDims: true,
      showTapeOut: true, showTravel: true, travelIn: 1.5, fullWords: true, textScale: 1.3,
      projectName: "Invariant", clientName: "QA", revision: "01",
      isMobile: false, isTablet: false, viewportW: 1280,
    };
    const layout = computeLayout({
      selectedSize: 75, brand: "Sony", centerH: 60, tvCL: 60, showBackBox: true,
      effectiveBoxModel: recommendBackBox(75, "sanus", "Sony"), mountSystem: "sanus",
      sanusMount: recommendSanusMount(75, "fixed", "Sony"),
    });
    // Flatten to every text node's position and size, ignoring colour props.
    const geom = (P) => {
      const schem = buildSchematic({ ...st, layout }, P);
      const out = [];
      const walk = (n) => {
        if (!n || typeof n !== "object") return;
        if (Array.isArray(n)) return n.forEach(walk);
        if (n.type === "text") out.push(`${n.props.x},${n.props.y},${n.props.fontSize},${n.props.textAnchor || ""}`);
        if (n.props && n.props.children) walk(n.props.children);
      };
      walk(schem.elements);
      return { dims: `${schem.svgW}x${schem.svgH}`, texts: out };
    };
    const a = geom(SCREEN_PALETTE), b = geom(CYANOTYPE_PALETTE), c = geom(PAPER_PALETTE);
    return a.texts.length > 20 &&
  ...
  T("obstruction", "exports carry it correctly, and it is not mistaken for ink", (() => {
    const mk = [{ id: "s", type: "obst", text: "shelf", clear: 3, color: "#FFFFFF",
                  pts: [{ x: 20, y: 30 }, { x: 60, y: 38 }] }];
    const st = { wallW: 120, wallH: 108, hasFireplace: false, fbOpeningW: 40, fbOpeningH: 30,
                 fbOffsetIn: 0, hasMantel: false, mantelH: 54, mantelDepth: 8, brand: "Sony",
                 selectedSize: 65, dispUnits: "dec", showVesa: true, showOutlet: true,
                 showLowVolt: true, heightRef: "center", projectName: "T", clientName: "", revision: "01" };
    const dxf = buildDXF({ ...st, markup: mk }, computeLayout({
      selectedSize: 65, brand: "Sony", centerH: 60, tvCL: 60, showBackBox: true,
      effectiveBoxModel: recommendBackBox(65, "sanus", "Sony"), mountSystem: "sanus", sanusMount: null }));
    // A rectangle on its own declared layer, not a diagonal on MARKUP.
    const declared = /LAYER\r?\n\s*2\r?\nOBSTRUCTION/.test(dxf);
    const onLayer = dxf.split(/\r?\n/).filter(l => l.trim() === "OBSTRUCTION").length;
    // and it must not be reported as a stroke that would vanish on paper
    const risk = markupContrastRisk(mk, PRINT_PALETTE);
    return declared && onLayer >= 2 && risk.length === 0
        && markupContrastRisk([{ id: "w", type: "rect", color: "#FFFFFF", pts: [{ x: 0, y: 0 }, { x: 1, y: 1 }] }], PRINT_PALETTE).length === 1;
  })());

  T("interop", "a design with obstructions round-trips to the same height", (() => {
    // The failure this pins: markup was exported and never read back, so
    // re-opening your own file moved the TV.
    const mk = [{ id: "s", type: "obst", text: "shelf", clear: 3, color: "#FF3B30", w: 2,
                  pts: [{ x: 40, y: 32 }, { x: 80, y: 36 }] }];
    const back = cleanImportedMarkup(mk);
    if (!back || back.length !== 1) return false;
    const args = { selectedSize: 65, brand: "Sony", hasFireplace: false, hasMantel: false,
                   mantelH: 54, fbOpeningH: 30, useViewDist: false, viewDist: 144, tvCL: 60 };
    const before = computeRecommendedCenterH({ ...args, obstructions: obstructionsFrom(mk) });
    const after = computeRecommendedCenterH({ ...args, obstructions: obstructionsFrom(back) });
    const { h } = tvDimsFor("Sony", 65);
    return approx(before, after) && approx(after, 36 + 3 + h / 2)
        && back[0].text === "shelf" && back[0].clear === 3;
  })());
  T("interop", "malformed markup is dropped, not trusted into state", (() => {
    const out = cleanImportedMarkup([
      { type: "obst", pts: [{ x: 1, y: 2 }, { x: 3, y: 4 }] },                 // ok, defaults filled
      { type: "obst", pts: [{ x: "1", y: 2 }, { x: 3, y: 4 }] },               // non-numeric point
      { type: "obst", clear: -5, pts: [{ x: 1, y: 2 }, { x: 3, y: 4 }] },      // bad clearance -> default
      { type: "laser", pts: [{ x: 1, y: 2 }, { x: 3, y: 4 }] },                // unknown type
      { type: "obst", pts: [{ x: 1, y: 2 }] },                                 // too few points
      { type: "obst", clear: Infinity, pts: [{ x: 0, y: 0 }, { x: 1, y: 1 }] },
      null, "nope", 7,
    ]);
    // survivors: the plain one, the negative-clearance one and the Infinity one —
    // all three keep their geometry and fall back to the default clearance.
    return out.length === 3
        && out.every(m => m.type === "obst" && MARKUP_SWATCHES.includes(m.color) && m.pts.length === 2)
        && out.every(m => m.clear === undefined)
        && cleanImportedMarkup(null) === null && cleanImportedMarkup("x") === null;
  })());
  ...
  T("obstruction", "a traced box becomes a rectangle in wall inches", (() => {
    ...
    // clearance may arrive as text (what the field stores) or a number (older
    // designs, and imports) — both must land on the same inches.
    const typed = obstructionsFrom([{ id: "t", type: "obst", clear: "6.5", pts: [{ x: 0, y: 0 }, { x: 10, y: 10 }] }]);
    const blank = obstructionsFrom([{ id: "b", type: "obst", clear: "", pts: [{ x: 0, y: 0 }, { x: 10, y: 10 }] }]);
    const junk = obstructionsFrom([{ id: "j", type: "obst", clear: "abc", pts: [{ x: 0, y: 0 }, { x: 10, y: 10 }] }]);
    if (!(typed[0].clear === 6.5 && blank[0].clear === CLEARANCE.obstruction && junk[0].clear === CLEARANCE.obstruction)) return false;
  ...
  // dimensions come back. The test knows the right answer independently — the
  // same shape of check that caught the WB21 transposition.
  T("rectify", "known oblique view is recovered, interior points included", (() => {
    const rect = [[0, 0], [100, 0], [100, 60], [0, 60]];
    const H = [1.2, 0.30, 10, -0.15, 1.10, 20, 0.0008, 0.0005, 1];   // a real perspective
    const shot = rect.map(([x, y]) => applyH(H, x, y));
    const back = solveHomography(shot, rect);
    if (!back) return false;
    // Corners fitting is trivial — four points exactly determine H. The claim
    // worth testing is that everything BETWEEN them lands correctly too.
    const probes = [[50, 30], [12.5, 47], [98, 3], [1, 59]];
    return probes.every(([x, y]) => {
      const q = applyH(back, ...applyH(H, x, y));
      return approx(q[0], x, 0.01) && approx(q[1], y, 0.01);
    });
  })());
  T("rectify", "affine case is exact (no perspective term)", (() => {
    const rect = [[0, 0], [80, 0], [80, 40], [0, 40]];
    const shot = rect.map(([x, y]) => [x * 2.5 + 17, y * 2.5 - 9]);
    const back = solveHomography(shot, rect);
    const q = applyH(back, 40 * 2.5 + 17, 20 * 2.5 - 9);
    return approx(q[0], 40, 1e-6) && approx(q[1], 20, 1e-6);
  })());
  T("rectify", "a real 55\" panel shot at an angle recovers its true size", (() => {
    // Sony 55" from the catalog, photographed obliquely.
    const d = tvDims(55);
    const H = [0.92, 0.18, 240, -0.09, 0.97, 300, 0.00035, 0.00021, 1];
    const quad = [[0, 0], [d.w, 0], [d.w, d.h], [0, d.h]].map(([x, y]) => applyH(H, x * 9, y * 9));
    const plan = planRectify(quad, 2400, 1600, d.w, d.h);
    if (!plan) return false;
    // The reference must span exactly realW x realH inches at the planned ppi.
    return approx(plan.refPx.w / plan.ppi, d.w, 0.01) && approx(plan.refPx.h / plan.ppi, d.h, 0.01)
        && Math.max(plan.outW, plan.outH) <= UNDERLAY_MAX_PX;
  })());
  T("rectify", "degenerate quad is refused, not fudged", (() => {
    const collinear = [[0, 0], [10, 0], [20, 0], [30, 0]];
    return solveHomography(collinear, [[0, 0], [1, 0], [1, 1], [0, 1]]) === null &&
           planRectify(collinear, 100, 100, 10, 10) === null &&
           planRectify([[0, 0], [10, 0], [10, 10], [0, 10]], 100, 100, 0, 10) === null;
  })());
  T("rectify", "guards fire on steep angles and tiny references", (() => {
    // Each quad isolates ONE guard: the square is large and square-on, the steep
    // one is large but oblique, the tiny one is square-on but small. A first
    // draft used a 100px square in a 1000px frame and tripped the coverage
    // warning while trying to prove no warning fires.
    const square = [[50, 50], [950, 50], [950, 950], [50, 950]];
    const steep = [[100, 100], [900, 100], [750, 900], [250, 900]];
    const okChecks = rectifyChecks(square, 1000, 1000);
    const steepChecks = rectifyChecks(steep, 1000, 1000);
    const tiny = rectifyChecks([[0, 0], [40, 0], [40, 40], [0, 40]], 2000, 2000);
    return okChecks.notes.length === 0
        && steepChecks.notes.length === 1 && /angle/i.test(steepChecks.notes[0])
...
// A VIEW filters the DRAWING and nothing else.
//
// `force` lists groups the view guarantees even when the Settings toggle is off.
// Only DERIVED AIDS may appear there — tape-out lines and the VESA pattern are
// computed from geometry already on the page, so drawing them adds no scope.
// Scope facts (outlet, low voltage, back box) are never forced: showOutlet also
// drives the parts list, so if the job has no outlet a page that drew one would
// be lying about the job. A page dedicated to a derived aid should never be
// blank; a page whose subject genuinely isn't in this job should be.
//
// It deliberately does NOT flip the Settings toggles. Those are design facts —
// showOutlet also drives the parts list and the DXF notes, so a "Mount" preset
// that switched it off would silently drop the outlet from that page's parts
// list. The job must describe the same scope on every sheet; only what is drawn
// changes.
const VIEW_PRESETS = {
  full:    { label: "Full",     hint: "everything",                 tvDims: 1, vesa: 1, box: 1, pwr: 1, lv: 1, tape: 1, mount: 1, force: {} },
  layout:  { label: "Layout",   hint: "TV, wall and heights",       tvDims: 1, vesa: 0, box: 0, pwr: 0, lv: 0, tape: 0, mount: 0, force: { tvDims: 1 } },
  roughin: { label: "Rough-in", hint: "box, power, low voltage",    tvDims: 0, vesa: 0, box: 1, pwr: 1, lv: 1, tape: 0, mount: 0, force: {} },
  mount:   { label: "Mount",    hint: "VESA, plate and hardware",   tvDims: 1, vesa: 1, box: 0, pwr: 0, lv: 0, tape: 0, mount: 1, force: { vesa: 1 } },
  tapeout: { label: "Tape-out", hint: "the four lines to snap",     tvDims: 0, vesa: 0, box: 0, pwr: 0, lv: 0, tape: 1, mount: 0, force: { tape: 1 } },
  // The client view is not a filter with everything switched off — it is a
  // different KIND of drawing. `chrome: 0` drops the wall outline, floor,
  // dimension lines and title block; `panel: 1` draws a television instead of a
  // rectangle. A client view of a blueprint is not persuasive; a client view of
  // their own wall is, and that is the whole point of the camera work.
  client:  { label: "Client",   hint: "their wall, their TV, nothing else",  tvDims: 0, vesa: 0, box: 0, pwr: 0, lv: 0, tape: 0, mount: 0, chrome: 0, panel: 1, force: {} },
};
const VIEW_ORDER = ["full", "layout", "roughin", "mount", "tapeout", "client"];
...
// Cyanotype — the classic blueprint: white linework burned onto a saturated
// blue ground. Accents are pale tints, because a mid-tone accent that reads
// well on near-black (the dark theme) disappears against this blue.
//
// Worth being honest about: this is a DARK palette. It is the picture everyone
// has of a blueprint, but it will not help in direct sun — that is what the
// paper theme is for.
const CYANOTYPE_PALETTE = {
  id: "cyn",
  canvas: "#12395E", grid: "rgba(255,255,255,0.07)",
  wallFill: "#17456F", wallStroke: "#FFFFFF",
  line: "#FFFFFF", lineSoft: "#A8C4DD",
  tvFill: "#0E2E4C", tvStroke: "#FFFFFF", screenFill: "#103552", tvLabel: "#A8C4DD",
  mantel: "#2A5A85", fbFill: "#1A4A76", fbOpen: "#0C2740",
  dimText: "#FFFFFF", dimSub: "#A8C4DD", halo: "#12395E", dimW: 1, maskFill: "#12395E",
  cl: "#FF9C8F", vesa: "#FFD98A", box: "#7FE3F0", boxBad: "#FF8A7A", pwr: "#8BEBA8", lv: "#FFC48A",
  mount: "#A8C4DD",
  pillStyle: "filled", pillText: "#0B2B47",
  tape: "#9BF3E4",
  title: "#A8C4DD",
};

// Paper — the print palette on screen, so what you see is what the PDF prints.
// Only the grid is lifted: the print grid is nearly invisible by design (it
// should not compete with linework on paper) but on a backlit screen it
// disappears entirely, and the grid is what makes the drawing read as scaled.
const PAPER_PALETTE = {
  ...PRINT_PALETTE,
  id: "ppr",
  canvas: "#FBFCFE",
  grid: "rgba(16,42,67,0.10)",
};

// The three themes. `palette` drives the DRAWING; `tokens` drive the app
// chrome. Both have to move together — a blueprint drawing inside dark-navy
// chrome looks like a bug, not a theme.
const THEMES = {
  dark:      { label: "Dark",      hint: "the original — near-black, blueline",  palette: SCREEN_PALETTE },
  blueprint: { label: "Blueprint", hint: "cyanotype — white lines on blue",      palette: CYANOTYPE_PALETTE },
  paper:     { label: "Paper",     hint: "what the PDF prints — ink on white",   palette: PAPER_PALETTE },
};
const THEME_ORDER = ["dark", "blueprint", "paper"];

// Density is the size counterpart to a theme, and deliberately uses the exact
// same mechanism — a token block toggled by an attribute on <html>. Compact
// reproduces the pre-2.7 metrics; it does NOT reproduce the sub-44pt tap
// targets, which were a defect rather than a density.
const DENSITIES = {
  comfortable: { label: "Comfortable", hint: "larger type, room to tap — the default on iPad" },
  compact:     { label: "Compact",     hint: "the older, tighter metrics — more on screen at once" },
};
...
const renderMarkupEls = (items, { wallX, floorY, scale, keyPrefix = "mk", fmt, paper, ink, ts = 1 }) => {
  ...
  (items || []).forEach((m, i) => {
    ...
    // `auto` becomes this palette's linework colour here and only here — the
    // stored stroke is left alone, so the same design prints black on the
    // submittal and draws white on the blueprint.
    const col = resolveMarkupColor(m.color, { line: ink });
    const sw = m.w || 2;
    const pts = m.pts || [];
...
const buildSchematic = (S, BASE_P) => {
  ...
  // Design flag AND view filter. Every drawing site below uses these, never the
  // raw flags, so a view can never alter what the job contains.
  const V = VIEW_PRESETS[S.view] || VIEW_PRESETS.full;
  const F = V.force || {};
  const vChrome = V.chrome !== 0;   // wall outline, floor, wall dims, title block
  const vPanel = !!V.panel;        // draw a television, not a wireframe
  // on(group, toggle): the view must allow the group, and either the user asked
  // for it or the view guarantees it (derived aids only — see VIEW_PRESETS).
  const on = (g, t) => !!V[g] && (!!t || !!F[g]);
  const vVesa   = on("vesa", showVesa);
  const vPwr    = on("pwr", showOutlet);
  const vLv     = on("lv", showLowVolt);
  const vTape   = on("tape", showTapeOut);
  const vTvDims = on("tvDims", showTvDims);
  const vBoxDim = on("box", showBoxDims);
  const vBox    = !!V.box;
  const vMount  = !!V.mount;
  const vTravel = showTravel  && !!V.mount;
  ...
  // Which callouts will actually register on the rail. hasRail (which reserves
  // HORIZONTAL space) and the packing simulation below (which reserves VERTICAL
  // space) must be driven by the same predicates: a pill that packs but wasn't
  // reserved lands on the wall-height dimension. That is exactly what happened
  // when Tape-out became the first view whose rail holds only a tape pill.
  const railHasVesa   = !!(layout && layout.vesa && vVesa);
  const railHasBox    = !!(layout && layout.box && vBox);
  const railHasSanus  = !!(layout && vMount && layout.mount?.system === "sanus");
  const railHasTravel = !!(layout && vTravel && S.mountSystem === "fa" && layout.box && layout.box.brand === "Future Automation" && travelIn > 0);
  const hasRail = !!layout && (railHasVesa || railHasBox || vPwr || vLv || vTape || railHasSanus || railHasTravel);
  ...
  if (layout) {
    ...
    const ents = [];
    if (railHasVesa) ents.push({ a: relY(layout.vesa.aff + layout.vesa.h / 2), n: 2 });
    if (railHasBox) ents.push({ a: relY(layout.box.aff), n: 2 + (layout.box.extendsOff ? 1 : 0) + (vBoxDim ? 2 + (hasFireplace && hasMantel ? 1 : 0) : 0) });
    if (vPwr) ents.push({ a: relY(layout.outlet.aff), n: 2 });
    if (vLv) ents.push({ a: relY(layout.lv.aff), n: 2 });
    if (vTape) ents.push({ a: relY(layout.tvTop), n: 2 });
    if (railHasSanus) ents.push({ a: relY(layout.vesa ? layout.vesa.aff : layout.centerH), n: 2 });
    if (railHasTravel) ents.push({ a: relY(layout.tvTop + travelIn), n: 2 });
    const relTvTop = relY(layout.tvTop);
    const topFloor = vTvDims ? relTvTop + 2 - 12 : 10;
    let prevTop = -Infinity, prevH = 0;
    ents.map((e, i) => ({ ...e, _i: i }))
  ...
  const elements = [];
  // Unique per palette AND view: the multi-page PDF puts several renders in one
  // document, and a shared id (the underlay clipPath) would make every page clip
  // to whichever page the browser parsed first.
  const K = (k) => `${P.id}-${S.view || "full"}-${k}`;
  ...
  if (hasUnderlay) {
    ...
    elements.push(<image key={K("underlay")} href={underlay.src} xlinkHref={underlay.src}
      ...
      opacity={vPanel ? 1 : (underlay.opacity ?? 0.75)} preserveAspectRatio="none"
  ...
  if (masks.length) {
    renderMarkupEls(masks, { wallX, floorY, scale, keyPrefix: K("mask"), fmt, paper: P.maskFill, ink: P.line, ts: TS })
  ...
  // The parametric fireplace is a stand-in for one nobody has photographed. Over
  // an actual photo of the actual fireplace it is not just redundant, it is
  // wrong — a symbol drawn on top of the thing it symbolises.
  if (hasFireplace && vChrome) {
  ...
  if (layout) {
    ...
    // TV
    if (vPanel) {
      // A television, not a rectangle. The client is being shown their own room;
      // an engineering wireframe floating on the photo undoes the point of
      // photographing it. Three things do almost all the work: a real drop
      // shadow so it sits ON the wall instead of over it, a near-black screen
      // (an off TV is very dark, not grey), and a sheen so the glass reads as
      // glass. Dimensions are deliberately absent — this sheet is not built from.
      const sid = K("shadow"), gid = K("glass");
      const r = Math.min(6, tvPxH * 0.04);
      elements.push(
        <defs key={K("panel-defs")}>
          <filter id={sid} x="-25%" y="-25%" width="150%" height="160%">
            <feDropShadow dx="0" dy={Math.max(2, tvPxH * 0.03)} stdDeviation={Math.max(3, tvPxH * 0.035)} floodColor="#000000" floodOpacity="0.45"/>
          </filter>
      ...
      elements.push(<rect key={K("tv")} x={tvX} y={tvY} width={tvPxW} height={tvPxH} rx={r} fill="#101215" filter={`url(#${sid})`}/>);
      const bez = Math.max(1.5, Math.min(4, tvPxW * 0.006));
      elements.push(<rect key={K("tvscreen")} x={tvX + bez} y={tvY + bez} width={Math.max(0, tvPxW - bez * 2)} height={Math.max(0, tvPxH - bez * 2)}
                          rx={Math.max(0, r - 1)} fill="#07080A"/>);
      elements.push(<rect key={K("tvglass")} x={tvX + bez} y={tvY + bez} width={Math.max(0, tvPxW - bez * 2)} height={Math.max(0, tvPxH - bez * 2)}
                          rx={Math.max(0, r - 1)} fill={`url(#${gid})`}/>);
      elements.push(<rect key={K("tvedge")} x={tvX} y={tvY} width={tvPxW} height={tvPxH} rx={r} fill="none" stroke="#000000" strokeOpacity="0.55" strokeWidth="1"/>);
    } else {
    ...
    if (layout.vesa) {
      ...
      if (vMount) elements.push(<rect key={K("mount")} x={vesaCenterX - mountPxW / 2} y={vesaCenterY - mountPxH / 2} width={mountPxW} height={mountPxH} fill="none" stroke={P.mount} strokeWidth="1.2" strokeDasharray="2 3" opacity="0.7"/>);
    }
    ...
    if (layout.box) {
      ...
      if (vBox) elements.push(<rect key={K("bb")} x={bbX} y={bbY} width={bbPxW} height={bbPxH} fill={c} fillOpacity="0.14" stroke={c} strokeWidth="1.8" strokeDasharray="8 4"/>);
      if (vBoxDim) {
    ...
    packRail(rail, vTvDims ? tvY + 2 : wallY + 10, TS).forEach(e => {
    ...
    // ----- dimensions -----
    // Suppressed entirely in the client view: they are being shown their room,
  ...
  if (vChrome) obstructionsFrom(markup).forEach((o, i) => {
  ...
  // "NOT TO SCALE — DIMENSIONS GOVERN" is a warning to whoever is building from
  // the sheet. The client view is not built from, and the caveat only reads as
  // hedging to someone being shown their own living room.
  if (vChrome) elements.push(<text key={K("nts")} x={svgW - 16} y={tbY} textAnchor="end" fill={P.title} fontSize={FS(9)} fontFamily="'IBM Plex Mono', monospace" letterSpacing="1">{ntsStr}</text>);
...
const sweepConfigs = () => {
  ...
  // ---- task views ------------------------------------------------------
  // Every config above renders the Full view (base has no `view`, and
  // buildSchematic falls back to full). A view repacks the leader rail from a
  // different subset, so a page that is fine at Full can still collide — and
  // Tape-out/Mount force their own guides on regardless of the toggles. Sweep
  // each view against the cases that stress label width: full words with ft-in
  // (the longest strings), a narrow phone viewport, a cramped wall, a giant
  // wall, and the largest text scale.
  const viewStress = [
    { n: "fullwords ftin fp", brand: "Sony", selectedSize: 65, mountSystem: "fa", dispUnits: "ftin", hasFireplace: true, fullWords: true },
    { n: "mobile 375", brand: "Samsung", selectedSize: 75, mountSystem: "sanus", sanusStyle: "fullmotion", dispUnits: "frac", hasFireplace: false, isMobile: true, viewportW: 375 },
    { n: "cramped 60x72", brand: "Samsung", selectedSize: 32, wallW: 60, wallH: 72, mountSystem: "fa", dispUnits: "ftin", hasFireplace: false, heightRef: "bottom", fullWords: true },
    { n: "giant wall 115", brand: "Samsung", selectedSize: 115, wallW: 300, wallH: 140, mountSystem: "sanus", sanusStyle: "fixed", dispUnits: "ftin", hasFireplace: false },
    { n: "XL text offset TV", brand: "LG", selectedSize: 55, mountSystem: "sanus", sanusStyle: "fixed", dispUnits: "dec", hasFireplace: false, tvOffsetIn: -25, textScale: TEXT_SCALES[TEXT_SCALES.length - 1].v },
  ];
  VIEW_ORDER.filter(v => v !== "full").forEach(view =>
    viewStress.forEach(c => cfgs.push({ ...base, ...c, name: `view:${view} ${c.n}`, view })));
...
// Sections inside a Stage are single-open. The provider carries the open key so
// Sec does not need to know it is in a group — outside one it keeps its own
// state, which is what the Data screen and any future standalone use rely on.
// React.useContext, not the bare hook: dev.html injects only useState/useMemo/
// useRef/useEffect into scope, so anything else has to come off React itself.
const SecGroup = React.createContext(null);

const Sec = ({ icon, title, children, first, defaultOpen = true, summary }) => {
  const grp = React.useContext(SecGroup);
  const [solo, setSolo] = useState(defaultOpen);
  const open = grp ? grp.open === title : solo;
  const toggle = () => (grp ? grp.set(grp.open === title ? null : title) : setSolo(o => !o));
  return (
    <div style={{ marginTop: grp ? 0 : (first ? 0 : 22) }}>
      <div className="sec-title sec-clk" onClick={toggle}>
...
// A STAGE is one step of the job. Only the active stage renders its controls;
// the rest collapse to a summary row you can tap to jump to. The sidebar used
// to stack ~34 rows — about 2.4 screens — before a TV was even chosen, which is
// the same "too much at once" problem the drawing views solved.
const Stage = ({ id, name, note, summary, active, onPick, defaultSec, children }) => {
  const [openSec, setOpenSec] = useState(defaultSec || null);
  return (
    <div className={`stg ${active ? "on" : ""}`}>
      <button type="button" className="stg-head" onClick={() => onPick(id)}
              title={active ? `${name} — open` : `Go to ${name}`}>
        <span className="stage-name">{name}</span>
        <span className="stage-note">{active ? note : (summary || note)}</span>
        <span className={`sec-chev ${active ? "" : "closed"}`}>▾</span>
      </button>
      {active && (
        <div className="stg-body">
          <SecGroup.Provider value={{ open: openSec, set: setOpenSec }}>{children}</SecGroup.Provider>
        </div>
      )}
    </div>
  );
...
export default function App() {
  ...
  const [view, setView] = useState(VIEW_PRESETS[SAVED.view] ? SAVED.view : "full");
  ...
  const [mkColor, setMkColor] = useState(MARKUP_SWATCHES.includes(SAVED.mkColor) ? SAVED.mkColor : MARKUP_DEFAULT_COLOR);
  ...
  // There was no breakpoint above 1024: the sidebar rendered at 320px on a
  // 13" iPad in landscape and on a 2560px desktop alike.
  const isWide = viewportW >= 1280;
  // 1280 is also where the markup bar can hold the full swatch row: at 44pt
  // per tile it needs ~1080px and wraps below that, stranding CLEAR on its
  // own line. Narrower than this, swatches collapse into the style menu.

  useEffect(() => {
    try {
      const { src, ...underlayMeta } = underlay || {};
      window.localStorage.setItem(STORAGE_KEY, JSON.stringify({
        markup, mkColor, mkWidth, trace, snapOn, textScale, view,
  ...
  const printSchem = useMemo(() => buildSchematic({ ...schemState, forPrint: true, isMobile: false, isTablet: false, viewportW: 1280 }, PRINT_PALETTE),
    [wallW, wallH, hasFireplace, fbOpeningW, fbOpeningH, fbOffsetIn, hasMantel, mantelH, mantelDepth,
     brand, selectedSize, layout, heightRef, mountSystem, showVesa, showOutlet, showLowVolt,
     showBoxDims, showTvDims, showTapeOut, showTravel, travelIn, fullWords,
     projectName, clientName, revision, dispUnits, underlay, markup, trace, catalogRev, textScale, view]);

  // PDF, PNG and SVG all serialise printRef, which is ALWAYS the print palette
  // on white — whatever the screen is showing. So the one ground that matters
  // for a delivered drawing is that white sheet, and this is the list of
  // strokes that would arrive on it invisible.
  const exportRisk = useMemo(() => markupContrastRisk(markup, PRINT_PALETTE), [markup]);
  ...
  useEffect(() => {
    ...
    // Keep the iOS status bar / PWA chrome in step with the app background.
    const meta = document.querySelector('meta[name="theme-color"]');
    if (meta) meta.setAttribute("content", theme === "paper" ? "#FFFFFF" : theme === "blueprint" ? "#0E2F4E" : "#0B1622");
  }, [theme, density]);
  ...
  const onPtrDown = (e) => {
    ...
    if (calib) {
      if (calib.mode === "quad") {
        // Four taps, in order: top-left, top-right, bottom-right, bottom-left.
        // Held in a ref because React batches a down/up pair into one render and
        // the fourth tap would otherwise read a stale list — the same trap the
        // two-point picker hit.
        const pts = [...calibPtsRef.current, q].slice(0, 4);
        calibPtsRef.current = pts;
        setCalib(c => c && ({ ...c, pts, hover: null }));
        if (pts.length === 4) setQuadAsk({ pts });
        return;
      }
      if (calib.mode === "box") {
  ...
  const onPtrMove = (e) => {
    ...
    if (calib) {
      ...
      else if (calib.mode === "quad" && calib.pts.length > 0 && calib.pts.length < 4) setCalib(c => c && ({ ...c, hover: q }));
      return;
    }
  ...
  const designState = {
    ...
    markup,
    // Calibration travels for reference but is NOT applied on import: without the
    // bitmap (which stays in IndexedDB) restoring a scale would leave a phantom
    // underlay. Annotations and obstructions DO come back.
  ...
  const applyImport = (extracted) => {
    ...
    // Markup was in the export all along and was never read back, so a design did
    // not round-trip: re-importing dropped every annotation, and once
    // obstructions existed that silently changed the mount height.
    const matched = [...extracted.matched];
    const mk = cleanImportedMarkup(f.markup);
    if (mk) {
      setMarkup(mk);
      const dropped = (Array.isArray(f.markup) ? f.markup.length : 0) - mk.length;
      const obs = mk.filter(m => m.type === "obst").length;
      if (mk.length) matched.push(`${mk.length} markup item${mk.length === 1 ? "" : "s"}${obs ? ` (${obs} obstruction${obs === 1 ? "" : "s"})` : ""}`);
      if (dropped > 0) notes.push(`${dropped} markup item${dropped === 1 ? "" : "s"} could not be read and were dropped`);
    }
    setImportSummary({ matched, ignored: extracted.ignored, notes, native: extracted.native });
  };
  ...
  // Serialize one view's print schematic without disturbing the live drawing.
  ...
  const renderViewSvg = (viewKey) => {
    ...
    const root = ReactDOM.createRoot(host);
    ReactDOM.flushSync(() => root.render(
      <svg width={schem.svgW} height={schem.svgH} viewBox={`0 0 ${schem.svgW} ${schem.svgH}`} xmlns="http://www.w3.org/2000/svg">
        <defs>
          <pattern id={`grid-${viewKey}`} width="20" height="20" patternUnits="userSpaceOnUse">
        ...
        {viewKey !== "client" && <rect width="100%" height="100%" fill={`url(#grid-${viewKey})`}/>}
        {viewKey !== "client" && <text x={schem.svgW - 16} y={18} textAnchor="end" fontSize="9" fill={PRINT_PALETTE.title} fontFamily="'IBM Plex Mono', monospace" letterSpacing="2">FRONT ELEVATION</text>}
        {viewKey !== "client" && <text x={16} y={18} fontSize="9" fill={PRINT_PALETTE.title} fontFamily="'IBM Plex Mono', monospace" letterSpacing="2">{selectedSize ? `${brand.toUpperCase()} ${selectedSize}"` : ""}</text>}
        {schem.elements}
      </svg>
    ));
    const out = new XMLSerializer().serializeToString(host.querySelector("svg"));
    root.unmount();
    host.remove();
    return out;
  };
  ...
  const exportPDF = () => {
    ...
    // Task sheets. Page 1 is always the whole job — the record drawing plus the
    // specs, parts and notes. Each sheet after it carries one task's drawing and
    // nothing else, so a crew member reads only the lines they are about to work
    // to. The spec and parts tables are NOT repeated: they describe the job, not
    // the sheet, and reprinting them per page invites someone to work from a
    // stale copy of a page they didn't notice was partial.
    // The client sheet is deliberately NOT in the pack: it is a different
    // artifact for a different person, and it carries no specifications. Sitting
    // in the client view and exporting gives you that one page on its own.
    const clientOnly = view === "client";
    const sheets = (multiPagePdf && !clientOnly)
      ? VIEW_ORDER.filter(v => v !== "full" && v !== "client").map(v => ({
          key: v, label: VIEW_PRESETS[v].label, hint: VIEW_PRESETS[v].hint, svg: renderViewSvg(v),
        }))
      : [];
    const sheetsHtml = sheets.map((sh, i) => `
<div class="sheet">
  <div class="header"><div><div style="font-size:9px;letter-spacing:2.5px;color:#5C7186;text-transform:uppercase;margin-bottom:4px;">SHEET ${i + 2} OF ${sheets.length + 1} · ${sh.label.toUpperCase()}</div><h1>${docTitle}</h1></div><div class="meta">${metaHtml}</div></div>
  <div class="sheet-note">${sh.label} — ${sh.hint}. Partial view: the specifications and parts list on sheet 1 govern the whole job.</div>
  <div class="schematic-wrap">${sh.svg}</div>
...
.sheet-note { font-size: 9.5px; letter-spacing: 0.6px; color: #5C7186; margin: 0 0 10px; text-transform: uppercase; }
...
<div class="header"><div><div style="font-size:9px;letter-spacing:2.5px;color:#5C7186;text-transform:uppercase;margin-bottom:4px;">${clientOnly ? "PROPOSED INSTALLATION" : `AV INSTALLATION DRAWING${sheets.length ? ` · SHEET 1 OF ${sheets.length + 1}` : ""}`}</div><h1>${docTitle}</h1></div><div class="meta">${clientOnly ? (clientName.trim() || "") : metaHtml}</div></div>
<div class="schematic-wrap"${clientOnly ? ' style="border:none;background:none;padding:0;"' : ""}>${sheets.length ? renderViewSvg("full") : svgData}</div>
${clientOnly ? "" : `<div class="legend-strip">${legendHtml}</div>`}
${clientOnly ? "" : `<div class="grid2">
...
<div class="footer"><span>${clientOnly ? "" : `Generated ${today} · REV ${revision || "01"}`}</span><span>${clientOnly ? "" : "NOT TO SCALE — DIMENSIONS GOVERN"}</span></div>
${sheetsHtml}
<script>window.addEventListener("load", function() { setTimeout(function() { window.print(); }, 500); });</script>
  ...
  // Only the three renderers that rasterise or serialise printRef are gated.
  // JSON stores every colour verbatim and DXF puts markup on its own layer
  // where the CAD app supplies the colour, so neither can lose a stroke.
  //
  // The guard REPORTS; it never rewrites. Someone who drew white over a dark
  // photo underlay meant it, and silently recolouring their annotation on the
  // way out would be a worse bug than the one this fixes. Recolouring is
  // offered as a button they press.
  const guardExport = (run, what) => {
    if (!exportRisk.length) { run(); return; }
    setExportAsk({ risk: exportRisk, run, what });
  };
  // Explicit, user-pressed remap. Only the strokes actually at risk change, and
  // they change to `auto` rather than to a fixed colour, so they stay legible
  // on whichever theme this design is opened on next.
  const autoInkRisky = () => {
    const bad = new Set(exportRisk.map(r => r.index));
    setMarkup(ms => ms.map((m, i) => (bad.has(i) ? { ...m, color: MARKUP_AUTO } : m)));
    setSel([]);
    setExportAsk(null);
  };
  ...
  const setupPanel = (
    <>
      <Stage id="start" name="START" note="what you are working from" defaultSec="Project"
             summary={[projectName, clientName].filter(Boolean).join(" — ") || "no project named"}
             active={stageOn("start")} onPick={pickStage}>
      ...
      <Stage id="wall" name="THE WALL" note="the surface it goes on" defaultSec="Wall"
             summary={`${fmt(wallW)} × ${fmt(wallH)}${hasFireplace ? (hasMantel ? " · fireplace + mantel" : " · fireplace") : ""}`}
             active={stageOn("wall")} onPick={pickStage}>
      ...
      <Stage id="tv" name="THE TV" note="what you are hanging" defaultSec="Brand"
             summary={selectedSize ? `${brand} ${selectedSize}"` : "no size chosen"}
             active={stageOn("tv")} onPick={pickStage}>
      ...
      <Stage id="hw" name="THE HARDWARE" note="what holds it up" defaultSec="Mount"
             summary={mountSystem === "fa" ? "Future Automation" : `Sanus ${sanusStyle === "fullmotion" ? "full motion" : sanusStyle}`}
             active={stageOn("hw")} onPick={pickStage}>
          ...
          <>
            ...
            {selectedSize && sanusMount && (
              <div className="rec-box">
                ...
                <div style={{ fontSize: "var(--fs-tag)", opacity: 0.6, marginTop: 3 }}>List ${sanusMount.list}</div>
              </div>
            )}
            {selectedSize && !sanusMount && (
        ...
        <Sec icon="bolt" title="VESA" summary={vesaSpec ? `${vesaSpec.w_mm}×${vesaSpec.h_mm}` : "no data"}>
          ...
          {vesaSpec && (
            <div className="vesa-box">
              ...
              <div style={{ fontSize: "var(--fs-tag)", marginTop: 6, paddingTop: 6, borderTop: "1px dashed currentColor", opacity: 0.8 }}>Verify with spec sheet before drilling</div>
            </div>
          )}
        </Sec>
      )}
      ...
      <div className="stg-all">
        <button type="button" className="chip" onClick={() => setStage(stage === "all" ? "start" : "all")}
                title={stage === "all" ? "Show one stage at a time" : "Show every section at once"}>
          {stage === "all" ? "BY STAGE" : "ALL SECTIONS"}
        </button>
      </div>
    </>
  );
  ...
  const settingsPanel = showSettings && (
    <div className="ask-wrap" onClick={() => setShowSettings(false)}>
      <div className="setp" onClick={e => e.stopPropagation()}>
        ...
        <SettingsRow label="Theme" hint={THEMES[theme].hint}>
          <Seg small options={THEME_ORDER.map(k => ({ value: k, label: THEMES[k].label.toUpperCase() }))}
               value={theme} onChange={setTheme}/>
        </SettingsRow>
        <SettingsRow label="Density" hint={DENSITIES[density].hint}>
          <Seg small options={DENSITY_ORDER.map(k => ({ value: k, label: DENSITIES[k].label.toUpperCase() }))}
        ...
        <SettingsRow label="Multi-page PDF" hint="One sheet per task — full drawing, then layout, rough-in, mount and tape-out">
          <Check on={multiPagePdf} onClick={() => setMultiPagePdf(!multiPagePdf)}>SHEETS</Check>
        </SettingsRow>
      ...
      <div className="obst-rows">
        {obstructions.map(o => (
          <div key={o.id} className="obst-row">
            ...
            <input className="inp" type="text" style={{ width: 92 }} placeholder={`${CLEARANCE.obstruction}" clear`}
                   value={o.clearRaw == null ? "" : String(o.clearRaw)}
                   onChange={e => setMarkup(ms => ms.map(m => m.id === o.id ? { ...m, clear: e.target.value } : m))}/>
  ...
  const diagPanel = showDiag && (
    <div className="diag-panel">
      ...
      {/* Derived from the results, not hard-coded: the list used to be a fixed
          four, so the markup and snapping groups ran on every load and passed
          into the badge count while never appearing in the panel. */}
      {[...new Set(selfTest.results.map(r => r.group))]
        .sort((a, b) => (DIAG_GROUP_ORDER.indexOf(a) + 1 || 99) - (DIAG_GROUP_ORDER.indexOf(b) + 1 || 99))
        .map(g => (
        <div key={g}>
  ...
  const exportBtns = (
    <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
      <div className="menu-wrap">
        ...
        {showExport && (
          <>
            ...
            <div className="menu">
              <button className="menu-item primary" onClick={() => { setShowExport(false); guardExport(exportPack, "FULL PACK"); }}>
                ★ FULL PACK<span className="menu-sub">PDF + JSON + DXF in one shot</span>
              </button>
              <div className="menu-sep"/>
              <button className="menu-item" onClick={() => { setShowExport(false); guardExport(exportPDF, "PDF"); }}>
              ...
              <div className="menu-sep"/>
              <button className="menu-item" onClick={() => { setShowExport(false); guardExport(exportPNG, "PNG"); }}>
                PNG<span className="menu-sub">blueline raster image</span>
              </button>
              <button className="menu-item" onClick={() => { setShowExport(false); guardExport(exportSVG, "SVG"); }}>
      ...
      {/* Standing, not modal, and outside the menu wrapper that anchors the
          dropdown. Visible while you are still drawing, which is when
          recolouring costs nothing. */}
      {exportRisk.length > 0 && (
        <button className="ink-warn" onClick={() => setExportAsk({ risk: exportRisk, run: null, what: null })}
                title="Some markup would be invisible in the PDF, PNG and SVG — click for the list">
          ▲ {exportRisk.length} INVISIBLE ON PAPER
        </button>
      )}
    </div>
  );
  ...
  const startBtns = (
    <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
        ...
        {showImport && (
          <>
            ...
            <div className="menu">
              ...
              <button className="menu-item" onClick={() => { setShowImport(false); photoFileRef.current && photoFileRef.current.click(); }}>
                PHOTOGRAPH THE WALL<span className="menu-sub">shoot it, then square up the perspective from something of known size</span>
              </button>
      ...
      <div className="menu-wrap">
        <button className={`btn ghost ${view !== "full" ? "flagged" : ""}`} onClick={() => setShowViewMenu(v => !v)}
                title="Filter the drawing for what you are doing right now">
          {VIEW_PRESETS[view].label.toUpperCase()} ▾
        </button>
        {showViewMenu && (
          <>
            <div className="menu-backdrop" onClick={() => setShowViewMenu(false)}/>
            <div className="menu">
              {VIEW_ORDER.map(k => (
                <button key={k} className={`menu-item ${view === k ? "primary" : ""}`}
                        onClick={() => { setView(k); setShowViewMenu(false); }}>
                  {VIEW_PRESETS[k].label}<span className="menu-sub">{VIEW_PRESETS[k].hint}</span>
                </button>
              ))}
              <div className="menu-sep"/>
              <div className="menu-note">Filters the drawing only — the parts list and specs always describe the whole job. Tape-out and Mount draw their own guides even if those are switched off in Settings.</div>
            </div>
          </>
        )}
      </div>
      ...
      {/* Always mounted, deliberately. These used to sit inside the Reference
          Drawing section and unmounted with it, leaving every entry point that
          calls .click() on the ref doing nothing at all. */}
      ...
      <input ref={photoFileRef} type="file" accept="image/*" capture="environment" style={{ display: "none" }}
             onChange={e => { const f = e.target.files && e.target.files[0]; if (f) importUnderlay(f); e.target.value = ""; }}/>
    </div>
  );
    ...
    <div className="startp">
      ...
      <div className="startp-row">
        <button className="startc" onClick={() => underlayFileRef.current && underlayFileRef.current.click()}>
          ...
          <span>Import the architect's elevation as a PDF, or photograph the wall and square up the perspective.</span>
        </button>
  ...
  const startCalib = (mode) => {
    ...
    setUnderlayNote(mode === "two"
      ...
      : mode === "quad"
      ? "Tap the four corners of something you know the size of — clockwise from the TOP-LEFT"
      : "Drag a box across the TV shown on the drawing");
  };

  // A swatch says two things: which colour it paints, and whether that colour
  // will still be there afterwards. The export sheet is ALWAYS the print
  // palette's white, whatever theme the screen is on, so "will it survive" is
  // two separate questions and the title answers whichever one is failing.
  const swatchNote = (c) => {
    if (c === MARKUP_AUTO) return "AUTO — follows the paper: light on the blueprint, dark on the printed sheet. Always survives export.";
    const okScreen = markupColorSafeOn(c, themePalette);
    const okPrint = markupColorSafeOn(c, PRINT_PALETTE);
    if (okScreen && okPrint) return c;
    if (!okScreen && !okPrint) return `${c} — nearly invisible in this theme AND on the white export sheet`;
    if (!okPrint) return `${c} — fine on screen, but invisible in the PDF, PNG and SVG (they always print on white)`;
    return `${c} — exports fine, but nearly invisible against this theme`;
  };
  const swatchBtn = (c) => {
    const auto = c === MARKUP_AUTO;
    const active = (solo ? solo.color : mkColor) === c;
    const risky = !auto && !(markupColorSafeOn(c, themePalette) && markupColorSafeOn(c, PRINT_PALETTE));
    return (
      <button key={c} type="button"
              className={`mk-sw ${active ? "on" : ""} ${risky ? "risky" : ""}`}
              style={auto
                ? { background: `linear-gradient(135deg, ${themePalette.line} 0 50%, ${PRINT_PALETTE.line} 50% 100%)` }
                : { background: c }}
              onClick={() => { setMkColor(c); if (sel.length) patchSel(m => ({ ...m, color: c })); }}
              title={sel.length ? `Recolour the selection — ${swatchNote(c)}` : swatchNote(c)}
              aria-label={auto ? "auto ink colour, follows the drawing" : `colour ${c}`}/>
    );
  };
      ...
      {isWide ? (
        ...
        <div className="menu-wrap">
          <button className="chip style-btn" onClick={() => setShowStyle(v => !v)} title="Pen colour and weight">
            <span className="mk-sw sm" style={(solo ? solo.color : mkColor) === MARKUP_AUTO
              ? { background: `linear-gradient(135deg, ${themePalette.line} 0 50%, ${PRINT_PALETTE.line} 50% 100%)` }
              : { background: solo ? solo.color : mkColor }}/>
  ...
  // Turn the four taps into a flat elevation. Everything here is measured in
  // wall inches and converted to native pixels once, because the underlay model
  ...
  const runRectify = async (refKey, customW, customH) => {
    ...
    if (refKey === "tv") {
      ...
      const r = RECTIFY_REFS.find(x => x.key === refKey);
      if (!r || !r.w) return;
      realW = r.w; realH = r.h; what = r.label;
    }
    const u = underlay;
    const toNative = (pt) => [(pt.x - u.ox) * u.ppi, (u.oy - pt.y) * u.ppi];
    const quad = quadAsk.pts.map(toNative);
    ...
    try {
      ...
      // Keep the reference where it already sat on the wall, so the image does
      // not jump. Scale is now real; POSITION is still yours to drag.
      const cx = quadAsk.pts.reduce((a, p) => a + p.x, 0) / 4;
      const cy = quadAsk.pts.reduce((a, p) => a + p.y, 0) / 4;
      const refCxPx = plan.refPx.x + plan.refPx.w / 2;
      const refCyPx = plan.refPx.y + plan.refPx.h / 2;
      const ox = cx - refCxPx / plan.ppi;
      const oy = cy + refCyPx / plan.ppi;
  ...
  const quadDialog = quadAsk && (
    <div className="ask-wrap" onClick={() => { if (!rectifying) { setQuadAsk(null); setCalib(null); calibPtsRef.current = []; } }}>
      <div className="ask" onClick={e => e.stopPropagation()} style={{ maxWidth: 520 }}>
        <div className="rec-tag">WHAT DID YOU JUST TAP?</div>
        <div className="hint" style={{ marginBottom: 10 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr auto", gap: 8, alignItems: "end" }}>
          <Field label="Width"><input className="inp" type="text" placeholder={`e.g. 36 or 3'`} value={quadAsk.w || ""}
                                      onChange={e => setQuadAsk(a => ({ ...a, w: e.target.value }))}/></Field>
          <Field label="Height"><input className="inp" type="text" placeholder={`e.g. 80`} value={quadAsk.h || ""}
                                       onChange={e => setQuadAsk(a => ({ ...a, h: e.target.value }))}/></Field>
          <button className="btn" disabled={rectifying || !quadAsk.w || !quadAsk.h}
                  style={{ marginBottom: 10 }} onClick={() => runRectify("custom", quadAsk.w, quadAsk.h)}>USE</button>
        </div>
        <div style={{ display: "flex", gap: 8, marginTop: 4, justifyContent: "flex-end" }}>
          <button className="btn ghost" disabled={rectifying}
                  onClick={() => { setQuadAsk(null); setCalib(null); calibPtsRef.current = []; }}>CANCEL</button>
        </div>
        {rectifying && <div className="hint" style={{ marginTop: 8 }}>Squaring the photo up…</div>}
      </div>
    </div>
  );
  ...
  // Named, not counted: "3 strokes are invisible" sends you hunting. The list
  // says which ones, in drawing order, each with the colour it was drawn in.
  const inkDialog = exportAsk && (() => {
    const risk = exportAsk.risk || [];
    const shown = risk.slice(0, 8);
    return (
      <div className="ask-wrap" onClick={() => setExportAsk(null)}>
        <div className="ask" style={{ width: "min(420px, 100%)" }} onClick={e => e.stopPropagation()}>
          <div className="rec-tag" style={{ color: "var(--warn)" }}>
            {risk.length} MARKUP {risk.length === 1 ? "STROKE" : "STROKES"} WON'T PRINT
            {exportAsk.what ? ` — ${exportAsk.what}` : ""}
          </div>
          <div className="hint" style={{ marginTop: 6, lineHeight: 1.55 }}>
            PDF, PNG and SVG always render on the white submittal sheet, whatever theme
            the screen is on. {risk.length === 1 ? "This stroke was" : "These were"} drawn
            in a colour too close to that white to see.
          </div>
          <div className="ink-list">
            {shown.map(r => (
              <div className="ink-row" key={`${r.id}-${r.index}`}>
                <span className="ink-chip" style={{ background: r.color }}/>
                <span className="ink-name">{r.label}</span>
                <span className="ink-hex">{r.color.toUpperCase()}</span>
              </div>
            ))}
            {risk.length > shown.length && (
              <div className="ink-row"><span className="ink-name">+ {risk.length - shown.length} more</span></div>
            )}
          </div>
          <div className="hint" style={{ marginTop: 8, lineHeight: 1.55 }}>
            If you drew light on purpose — over a dark photo underlay, say — export anyway;
            nothing here changes your drawing unless you ask it to.
          </div>
          <div style={{ display: "flex", gap: 8, marginTop: 12, justifyContent: "flex-end", flexWrap: "wrap" }}>
            <button className="btn ghost" onClick={() => setExportAsk(null)}>CANCEL</button>
            <button className="btn ghost" onClick={autoInkRisky}
                    title="Switch just these strokes to AUTO — dark on paper, light on the blueprint">
              SET TO AUTO INK
            </button>
            {exportAsk.run && (
              <button className="btn" title={`Export the ${exportAsk.what} with these strokes as drawn`}
                      onClick={() => { const go = exportAsk.run; setExportAsk(null); go(); }}>
                EXPORT ANYWAY
              </button>
            )}
          </div>
  ...
  // The in-progress stroke draws on the SCREEN palette, so it must be handed
  // that palette's ink and paper — otherwise an `auto` stroke would flip colour
  // the instant the drag ended and it moved into the schematic.
  const frame = { wallX: screenSchem.wallX, floorY: screenSchem.floorY, scale: screenSchem.scale, fmt,
                  paper: screenSchem.P.maskFill, ink: screenSchem.P.line };
  const calibOverlay = calib && (() => {
    ...
    if (calib.mode === "quad") {
      ...
      const live = pts.length < 4 && calib.hover ? [...pts, calib.hover] : pts;
      if (live.length >= 3) {
        els.push(<polygon key="cq" points={live.map(q => { const c = toPx(q); return `${c.x},${c.y}`; }).join(" ")}
          fill="rgba(74,158,255,0.12)" stroke="#4A9EFF" strokeWidth="1.5" strokeDasharray="5 3"/>);
      } else if (live.length === 2) {
        const a = toPx(live[0]), b = toPx(live[1]);
        els.push(<line key="cql" x1={a.x} y1={a.y} x2={b.x} y2={b.y} stroke="#4A9EFF" strokeWidth="1.5" strokeDasharray="5 3"/>);
      }
      pts.forEach((q, i) => {
        const c = toPx(q);
        els.push(<circle key={`cqp${i}`} cx={c.x} cy={c.y} r="6" fill="#4A9EFF" stroke="#FFFFFF" strokeWidth="1.5"/>);
        els.push(<text key={`cqn${i}`} x={c.x} y={c.y + 3.5} textAnchor="middle" fill="#FFFFFF"
          fontSize="9" fontWeight="700" fontFamily="'IBM Plex Mono', monospace">{i + 1}</text>);
      });
      if (pts.length < 4) {
        const c = calib.hover ? toPx(calib.hover) : null;
        if (c) els.push(<text key="cqh" x={c.x} y={c.y - 14} textAnchor="middle" fill="#4A9EFF"
          fontSize="11" fontWeight="600" fontFamily="'IBM Plex Mono', monospace">{CORNERS[pts.length]}</text>);
      }
    }
    if (calib.mode === "two") {
  ...
  const canvas = (
    <div className="canvas-panel">
      ...
      <svg ref={svgRef} width={screenSchem.svgW} height={screenSchem.svgH}
        ...
        {view !== "client" && <rect width="100%" height="100%" fill="url(#grid-scr)"/>}
        {view !== "client" && <text x={screenSchem.svgW - 16} y={18} textAnchor="end" fontSize="9" fill={screenSchem.P.title} fontFamily="'IBM Plex Mono', monospace" letterSpacing="2">FRONT ELEVATION</text>}
        {view !== "client" && <text x={16} y={18} fontSize="9" fill={screenSchem.P.title} fontFamily="'IBM Plex Mono', monospace" letterSpacing="2">{selectedSize ? `${brand.toUpperCase()} ${selectedSize}"` : "SELECT TV SIZE"}</text>}
  ...
  const printSvg = (
    <div style={{ position: "absolute", left: -100000, top: 0, width: 0, height: 0, overflow: "hidden" }} aria-hidden="true">
      <svg ref={printRef} width={printSchem.svgW} height={printSchem.svgH}
        ...
        {view !== "client" && <text x={printSchem.svgW - 16} y={18} textAnchor="end" fontSize="9" fill={PRINT_PALETTE.title} fontFamily="'IBM Plex Mono', monospace" letterSpacing="2">FRONT ELEVATION</text>}
        {view !== "client" && <text x={16} y={18} fontSize="9" fill={PRINT_PALETTE.title} fontFamily="'IBM Plex Mono', monospace" letterSpacing="2">{selectedSize ? `${brand.toUpperCase()} ${selectedSize}"` : ""}</text>}
        {printSchem.elements}
      </svg>
    </div>
  );
  ...
  return (
    <div className="app">
      <style>{`
        ...
        /* Dark is the default token set; a theme overrides only what changes.
           --warn-txt / --amber-txt / --acc-txt exist because the tinted notice
           boxes used to hardcode pale tints (#FFB4AC, #F0D9A0, #B8E8EF) that
           are legible on near-black and invisible on paper. */
        :root {
          --ink: #0B1622; --ink2: #0D1B2A; --panel: #112236; --panel2: #16293F;
          --line: rgba(232,238,245,0.16); --line2: rgba(232,238,245,0.18);
          --txt: #E8EEF5; --txt2: #A9BDD0; --txt3: #7E99B0;
          --acc: #3ECFE0; --warn: #FF5C4D; --ok: #4ADE80; --amber: #FFD166;
          --warn-txt: #FFB4AC; --amber-txt: #F0D9A0; --acc-txt: #B8E8EF; --sw-brd: rgba(232,238,245,0.30);
          --fd: 'Space Grotesk', sans-serif; --fm: 'IBM Plex Mono', monospace;
        }
        /* Cyanotype: white on saturated blue. Chrome sits a shade darker than
           the drawing canvas (#12395E) so the drawing reads as the lit surface. */
        :root[data-theme="blueprint"] {
          --ink: #0E2F4E; --ink2: #12395E; --panel: #16456C; --panel2: #1B5180;
          --line: rgba(255,255,255,0.18); --line2: rgba(255,255,255,0.24);
          --txt: #F2F8FD; --txt2: #C3DCF0; --txt3: #9DC9E8;
          --acc: #7FE3F0; --warn: #FF8A7A; --ok: #8BEBA8; --amber: #FFD98A;
          --warn-txt: #FFC9C0; --amber-txt: #FFE3B0; --acc-txt: #C4F1F8; --sw-brd: rgba(255,255,255,0.38);
        }
        /* Paper: the print palette on screen. Accents are the darkened print
           accents — the screen accents are far too light on white. */
        :root[data-theme="paper"] {
          --ink: #FFFFFF; --ink2: #F4F7FA; --panel: #FFFFFF; --panel2: #EDF2F7;
          --line: rgba(16,42,67,0.16); --line2: rgba(16,42,67,0.22);
          --txt: #102A43; --txt2: #41556A; --txt3: #5A6E82;
          --acc: #0A6273; --warn: #C0392B; --ok: #1E7D3C; --amber: #7A5D16;
          --warn-txt: #8A2A1E; --amber-txt: #6B5214; --acc-txt: #0A5C6C; --sw-brd: rgba(16,42,67,0.42);
        }
        /* ── Type scale and density ───────────────────────────────────────
           Colour lives in the theme blocks above; SIZE lives here. Two sets,
           one shape — the same technique, so a density is as cheap as a theme.

           The rule the scale encodes: MONO for values and identifiers
           (dimensions, model numbers, the status bar), SANS for prose (labels,
           hints, descriptions). The blueprint character belongs on the data,
           not on the chrome.

           Tracking is deliberately low. The previous scale put 1.5–2.5px on
           9px type — 17–28% of the em. Letterspacing aids uppercase to roughly
           10% and costs legibility past it, so nothing here exceeds that.

           --tap/--tap-sm are the iPadOS 44pt minimum. They shrink far less
           than the type does in Compact: a control too small to hit reliably
           is a defect at any density, not a preference. */
        :root {
          --fs-h1: 17px;   --fs-title: 11px;  --fs-label: 12px;  --fs-value: 16px;
          --fs-body: 14px; --fs-stat: 12px;   --fs-hint: 11.5px; --fs-num: 20px;
          --fs-btn: 11px;  --fs-chip: 10.5px; --fs-tag: 9.5px;
          --tr-title: 1px; --tr-label: 0;     --tr-btn: 1px;
          --tr-chip: 0.8px; --tr-tag: 1.6px;
          --tap: 44px;     --tap-sm: 40px;
          --sp-1: 6px; --sp-2: 10px; --sp-3: 14px; --sp-4: 20px; --sp-5: 24px;
        }
        :root[data-density="compact"] {
          --fs-h1: 16px;   --fs-title: 10px;  --fs-label: 11px;  --fs-value: 15px;
          --fs-body: 13px; --fs-stat: 11px;   --fs-hint: 10.5px; --fs-num: 18px;
          --fs-btn: 10px;  --fs-chip: 9.5px;  --fs-tag: 9.5px;
          --tap: 44px;     --tap-sm: 40px;
          --sp-1: 4px; --sp-2: 7px; --sp-3: 10px; --sp-4: 14px; --sp-5: 18px;
        }
        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        input[type="range"] { width: 100%; height: var(--tap); background: transparent; -webkit-appearance: none; appearance: none; margin: 0; }
        input[type="range"]::-webkit-slider-runnable-track { height: 3px; background: var(--line2); border-radius: 2px; }
        input[type="range"]::-webkit-slider-thumb { -webkit-appearance: none; width: 22px; height: 22px; border-radius: 50%; background: var(--acc); border: none; margin-top: -9.5px; }
        input[type="range"]::-moz-range-track { height: 3px; background: var(--line2); border-radius: 2px; }
        input[type="range"]::-moz-range-thumb { width: 22px; height: 22px; border-radius: 50%; background: var(--acc); border: none; }
        ...
        .hdr h1 { font-size: var(--fs-h1); font-weight: 700; letter-spacing: 0.3px; margin: 0; }
        .hdr .sub { font-family: var(--fm); font-size: var(--fs-tag); letter-spacing: var(--tr-tag); color: var(--txt3); margin-top: 2px; }
        .hdr-proj { font-family: var(--fm); font-size: var(--fs-stat); color: var(--txt2); }
        .sec-title { font-family: var(--fm); font-size: var(--fs-title); letter-spacing: var(--tr-title); text-transform: uppercase; color: var(--txt2); font-weight: 600; padding: var(--sp-1) 0; border-bottom: 1px solid var(--line); margin-bottom: var(--sp-2); min-height: var(--tap); display: flex; align-items: center; gap: 7px; }
        ...
        .lbl { font-family: var(--fd); font-size: var(--fs-label); letter-spacing: var(--tr-label); color: var(--txt2); margin-bottom: var(--sp-1); }
        .hint { font-family: var(--fd); font-size: var(--fs-hint); color: var(--txt3); margin-top: 4px; line-height: 1.5; }
        .inp { width: 100%; padding: 9px 11px; border: 1px solid var(--line2); background: var(--ink2); font-family: var(--fm); font-size: var(--fs-value); color: var(--txt); outline: none; border-radius: 4px; min-height: var(--tap); transition: border-color .15s; -webkit-appearance: none; appearance: none; }
        ...
        .chk-row { display: flex; align-items: center; gap: 10px; min-width: var(--tap); padding: 8px 0; cursor: pointer; font-size: var(--fs-body); min-height: var(--tap); user-select: none; -webkit-user-select: none; color: var(--txt); }
        ...
        .seg-btn { flex: 1; padding: 8px 10px; border: none; background: transparent; color: var(--txt2); font-family: var(--fm); font-size: var(--fs-chip); letter-spacing: var(--tr-chip); cursor: pointer; border-radius: 3px; transition: all .15s; min-height: var(--tap); min-width: var(--tap); }
        .seg-btn.on { background: var(--panel2); color: var(--txt); box-shadow: inset 0 0 0 1px var(--line2); }
        .seg.small .seg-btn { padding: 4px 8px; font-size: var(--fs-chip); min-height: var(--tap); }
        .stat { display: flex; justify-content: space-between; padding: 4px 0; font-size: var(--fs-stat); font-family: var(--fm); color: var(--txt2); gap: 8px; }
        .stat strong { color: var(--txt); font-weight: 600; }
        .stat.dim { opacity: 0.6; }
        .warn-box { margin-top: 10px; padding: 9px 11px; background: rgba(255,92,77,0.08); border: 1px solid rgba(255,92,77,0.5); border-radius: 4px; color: var(--warn-txt); font-family: var(--fm); font-size: var(--fs-hint); line-height: 1.55; }
        .warn-title { font-size: var(--fs-tag); letter-spacing: var(--tr-tag); color: var(--warn); margin-bottom: 4px; font-weight: 700; }
        .note-box { margin-top: 8px; padding: 8px 10px; background: rgba(255,209,102,0.07); border: 1px solid rgba(255,209,102,0.4); border-radius: 4px; color: var(--amber-txt); font-family: var(--fm); font-size: var(--fs-hint); line-height: 1.5; }
        .rec-box { margin-top: 8px; padding: 9px 11px; background: var(--panel2); border: 1px solid var(--line2); border-radius: 4px; font-family: var(--fm); font-size: var(--fs-hint); line-height: 1.5; color: var(--txt); }
        .rec-tag { font-size: var(--fs-tag); letter-spacing: var(--tr-tag); color: var(--acc); margin-bottom: 3px; font-weight: 700; }
        .vesa-box { margin-top: 8px; padding: 9px 11px; background: rgba(255,209,102,0.06); border: 1px solid rgba(255,209,102,0.35); border-radius: 4px; font-family: var(--fm); font-size: var(--fs-hint); line-height: 1.5; color: var(--amber-txt); }
        ...
        .strip-title { font-family: var(--fm); font-size: var(--fs-tag); letter-spacing: var(--tr-tag); color: var(--txt2); font-weight: 600; }
        ...
        .size-num { font-family: var(--fd); font-size: var(--fs-num); font-weight: 700; line-height: 1; }
        .size-sub { font-family: var(--fm); font-size: var(--fs-tag); letter-spacing: var(--tr-tag); margin-top: 4px; color: var(--txt3); }
        .chip { padding: var(--sp-1) var(--sp-2); border: 1px solid var(--line2); background: transparent; color: var(--txt2); font-family: var(--fm); font-size: var(--fs-chip); letter-spacing: var(--tr-chip); cursor: pointer; border-radius: 3px; min-height: var(--tap); min-width: var(--tap); transition: all .15s; }
        ...
        /* A colour that will vanish on the export sheet, or against this theme.
           A dot, not a slash across the chip: the swatch still has to read as
           the colour it paints — the mark is a caution, not a prohibition. */
        .mk-sw.risky::after { content: ""; position: absolute; right: -4px; top: -4px; width: 8px; height: 8px; border-radius: 50%; background: var(--warn); border: 1.5px solid var(--ink2); pointer-events: none; }
        .mk-sw.sm.risky::after { right: -3px; top: -3px; width: 6px; height: 6px; border-width: 1px; }
        ...
        .sidebar { padding-left: max(var(--sp-4), env(safe-area-inset-left)); }
        .main-col { padding-right: max(var(--sp-3), env(safe-area-inset-right)); }
        ...
        .stg-head { display: flex; align-items: center; gap: var(--sp-2); width: 100%; min-height: var(--tap);
                    padding: var(--sp-2) 0; background: transparent; border: none; cursor: pointer; text-align: left; color: inherit; }
        .stg-head .stage-note { flex: 1; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
        .stg:not(.on) .stg-head .stage-name { color: var(--txt3); }
        .stg-head:hover .stage-name { color: var(--acc); }
        .stg-body { padding-bottom: var(--sp-3); }
        .stg-all { margin-top: var(--sp-4); display: flex; justify-content: center; }
        ...
        .stage-name { font-family: var(--fm); font-size: var(--fs-tag); letter-spacing: var(--tr-tag); font-weight: 700; color: var(--acc); }
        .stage-note { font-size: var(--fs-hint); color: var(--txt2); opacity: 0.75; }
        .menu-note { padding: 8px 12px; font-size: var(--fs-hint); color: var(--txt2); opacity: 0.8; }
        ...
        .startc strong { display: block; color: var(--txt); font-size: var(--fs-body); margin-bottom: 3px; }
        .startc span { display: block; color: var(--txt2); font-size: var(--fs-hint); line-height: 1.45; }
        .main-col.dropping { outline: 2px dashed var(--acc); outline-offset: -6px; border-radius: 6px; }

        .btn.icon-btn { padding: 0 var(--sp-3); min-height: var(--tap); min-width: var(--tap); font-size: 14px; line-height: 1; }
        .style-btn { display: inline-flex; align-items: center; gap: 6px; }
        .mk-sw.sm { width: 13px; height: 13px; border-width: 1px; }
        /* Markup that will not survive the export sheet. Tinted like .warn-box
           rather than solid: it is a caution beside a working button, not an
           error state, and it must not read as "export is broken". */
        .ink-warn { display: inline-flex; align-items: center; gap: 6px; padding: 7px 10px; background: rgba(255,92,77,0.10); border: 1px solid var(--warn); border-radius: 5px; color: var(--warn-txt); font-family: var(--fm); font-size: 10px; letter-spacing: 1px; font-weight: 700; cursor: pointer; }
        .ink-warn:hover { background: rgba(255,92,77,0.18); }
        .ink-list { margin-top: 10px; border: 1px solid var(--line); border-radius: 4px; max-height: 168px; overflow-y: auto; }
        .ink-row { display: flex; align-items: center; gap: 9px; padding: 6px 9px; border-bottom: 1px solid var(--line); font-family: var(--fm); font-size: 11px; }
        .ink-row:last-child { border-bottom: none; }
        .ink-chip { flex: 0 0 auto; width: 13px; height: 13px; border-radius: 3px; border: 1px solid var(--sw-brd); }
        .ink-name { flex: 1 1 auto; color: var(--txt); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
        .ink-hex { flex: 0 0 auto; color: var(--txt3); font-size: 10px; letter-spacing: 0.5px; }
        ...
        .set-group { font-family: var(--fm); font-size: var(--fs-tag); letter-spacing: var(--tr-tag); font-weight: 700; color: var(--acc);
        ...
        .set-label { font-size: var(--fs-label); color: var(--txt); }
        .set-hint { font-size: var(--fs-hint); color: var(--txt2); margin-top: 2px; line-height: 1.4; }
        ...
        .dcell { width: 92px; padding: 4px 6px; min-height: var(--tap-sm); background: var(--ink); border: 1px solid var(--line2); border-radius: 3px;
        ...
        .derr { color: var(--warn); font-size: var(--fs-hint); margin-top: 3px; }
        ...
        .import-note { display: flex; gap: 12px; align-items: flex-start; padding: 10px 12px; background: rgba(62,207,224,0.07); border: 1px solid rgba(62,207,224,0.4); border-radius: 5px; font-family: var(--fm); color: var(--acc-txt); line-height: 1.5; }
        ...
        .menu-item { display: block; width: 100%; text-align: left; padding: 9px 12px; background: transparent; border: none; color: var(--txt); font-family: var(--fm); font-size: var(--fs-btn); letter-spacing: var(--tr-btn); cursor: pointer; border-radius: 4px; min-height: var(--tap); }
        .menu-item:hover { background: var(--panel2); }
        .menu-item.primary { color: var(--acc); font-weight: 700; }
        .menu-sub { display: block; font-size: var(--fs-hint); color: var(--txt3); letter-spacing: 0.3px; margin-top: 2px; font-weight: 400; }
        ...
        .status-vals { display: flex; gap: 14px; flex-wrap: wrap; font-family: var(--fm); font-size: var(--fs-stat); }
        ...
        .diag-row { display: flex; justify-content: space-between; gap: 10px; padding: 3px 0; font-family: var(--fm); font-size: var(--fs-hint); color: var(--txt2); border-bottom: 1px solid var(--line); }
        .diag-row.head-row { color: var(--txt); }
        .diag-row .p { color: var(--ok); }
        .diag-row .f { color: var(--warn); }
        .btn { display: inline-flex; align-items: center; gap: 6px; padding: 9px 16px; border: 1px solid var(--acc); background: var(--acc); color: var(--ink); font-family: var(--fm); font-size: var(--fs-btn); letter-spacing: var(--tr-btn); font-weight: 600; cursor: pointer; border-radius: 4px; min-height: var(--tap); transition: all .15s; }
        ...
        .tab { flex: 1; padding: 0 8px; min-height: var(--tap); border: none; background: transparent; border-bottom: 2px solid transparent; font-family: var(--fm); font-size: var(--fs-btn); letter-spacing: var(--tr-tag); cursor: pointer; color: var(--txt3); font-weight: 600; }
        ...
        .main-col { padding: var(--sp-3); display: flex; flex-direction: column; gap: 14px; min-width: 0; overflow-y: auto; }
        ...
        .legend-row { display: flex; gap: 8px; font-family: var(--fm); font-size: var(--fs-hint); color: var(--txt2); padding: 2px 0; }
        ...
        .mini-tag { font-family: var(--fm); font-size: var(--fs-tag); letter-spacing: var(--tr-tag); color: var(--txt3); text-align: center; margin-top: 4px; }
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : (isTablet ? "300px 1fr" : isWide ? "380px 1fr" : "336px 1fr"), flex: 1, minHeight: 0 }}>
