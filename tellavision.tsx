...
const VIEW_ORDER = ["full", "layout", "roughin", "mount", "tapeout"];
...
export default function App() {
  ...
  const renderViewSvg = (viewKey) => {
    ...
    ReactDOM.flushSync(() => root.render(
      <svg width={schem.svgW} height={schem.svgH} viewBox={`0 0 ${schem.svgW} ${schem.svgH}`} xmlns="http://www.w3.org/2000/svg">
        ...
        <rect width="100%" height="100%" fill={`url(#grid-${viewKey})`}/>
        <text x={schem.svgW - 16} y={18} textAnchor="end" fontSize="9" fill={PRINT_PALETTE.title} fontFamily="'IBM Plex Mono', monospace" letterSpacing="2">FRONT ELEVATION</text>
  ...
  const exportPDF = () => {
    ...
    const sheets = multiPagePdf
      ? VIEW_ORDER.filter(v => v !== "full").map(v => ({
...
<div class="header"><div><div style="font-size:9px;letter-spacing:2.5px;color:#5C7186;text-transform:uppercase;margin-bottom:4px;">AV INSTALLATION DRAWING${sheets.length ? ` · SHEET 1 OF ${sheets.length + 1}` : ""}</div><h1>${docTitle}</h1></div><div class="meta">${metaHtml}</div></div>
<div class="schematic-wrap">${sheets.length ? renderViewSvg("full") : svgData}</div>
<div class="legend-strip">${legendHtml}</div>
<div class="grid2">
